...
#        self.theta_fact=np.zeros(self._n)
        for i in range(self._n):
            ...
            samples,theta_name,self.theta_fit,self.theta_fact,self.minkaf[i],self.data_num[i]=np.load(savefile_name)
            self.Samp.append(MCSamples(samples=samples,names = theta_name, labels = theta_name))
        self.param_names=[]
